...
# The first argument (index 0) is the script name, and the second argument (index 1) is the variable
if len(command_line_args) >= 2:
    pin = command_line_args[1]
    pin = "`"+pin+"`"
...
#             )
#             end_date = start_date + datetime.timedelta(days=6)
#             query = "SELECT * FROM {} WHERE date >= %s AND date <= %s".format(pin)
#             cursor.execute(query, (start_date.date(), end_date.date()))
#         else:
#             month = selected_date_obj.month
#             query = "SELECT * FROM {} WHERE month(date) = %s".format(pin)
